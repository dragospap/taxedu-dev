...
jQuery(document).ready(function () {
    jQuery('.what-is-tax-menu').click(function(){
      ...
      // Checks if it is already active.
        var isActive = (parent.find('#img_up').hasClass('active')) ? true : false;
        // console.log(isActive);
        jQuery('.what-is-tax-menu').find('#img_up').removeClass('active')
        jQuery(this).addClass("active");
      if (!isActive) {
        parent.find('#img_up').addClass('active');
      }
    });
    ...
    jQuery("#map-europe").CSSMap({
        ...
        "onClick": function (e) {
          ...
          if (jQuery('#' + rLink + ' .col2').html() === '&nbsp;') {
              jQuery('#' + rLink + ' .col2').css('display','none');
            ...
            jQuery('#content_map').html('' +
                '<div class="title">' + rName + '</div><div class="redbar"></div><div class="description_map"><p>' +
                 jQuery('#' + rLink).html() +
                '</p></div> ');
        }
    });
