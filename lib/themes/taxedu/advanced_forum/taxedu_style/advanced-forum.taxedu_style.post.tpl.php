...
<div id="<?php print $post_id; ?>" class="<?php print $classes; ?>" <?php print $attributes; ?>>
  <div class="forum-post-info clearfix">
    <div class="forum-posted-on">
      <?php print date('d.m.Y H:m', ($top_post) ? $node->created : $date) ?>
  ...
  <div class="forum-post-wrapper">
    <div class="forum-post-panel-sub">
        ...
        <?php if($public_profile){
          echo l($account->name,'public/profile/'.$account->uid);
        }
        else {echo $account->name;}
        ?>
        </div>
        <div class="info">
            <?php echo t('Country: ').$country->field_country_name[LANGUAGE_NONE][0]['value']; ?>
            <?php echo t('Joined: ').date('d.m.y',$profile['created']) ?>
            <?php echo t('Posts: @number',array('@number'=>$stats['topics']))  ?>
        </div>
    </div>
